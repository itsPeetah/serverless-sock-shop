(function () {
  ...
  app.post("/orders", function (req, res, next) {
    ...
    console.log(
      "Posting orders request received with body: " + JSON.stringify(req.body)
    );
    var logged_in = req.cookies.logged_in;
    if (!logged_in) {
      throw new Error("User not logged in.");
          ...
          request(
            ...
            function (error, response, body) {
              ...
              try {
                ...
                // THIS IS JANK/A SHORTCUT TAKEN WHILE TESTING
                // The user service returns hrefs that point to the function service, not the dispatcher.
                // To enforce every call to function passing through the dispatcher/load balancer
                // I'm patching the return url to redirect it to said service.
                customerlink = patcher.patch_function_url(customerlink);
                addressLink = patcher.patch_function_url(addressLink);
                cardLink = patcher.patch_function_url(cardLink);
              ...
              function (callback) {
                ...
                request.get(addressLink, function (error, response, body) {
                  ...
                  if (
                    ...
                    const orderAddressLink = patcher.patch_function_url(
                      jsonBody._embedded.address[0]._links.self.href
                    );
                    order.address = orderAddressLink;
                  }
                  callback();
                });
              },
              function (callback) {
                ...
                request.get(cardLink, function (error, response, body) {
                  ...
                  if (
                    ...
                    const orderCardLink = patcher.patch_function_url(
                      jsonBody._embedded.card[0]._links.self.href
                    );
                    order.card = orderCardLink;
                  }
                  callback();
                });
              },
